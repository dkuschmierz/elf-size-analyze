...
from elfSizeAnalyze.misc.section import Section
from elfSizeAnalyze.misc.symbol import Symbol, SymbolsTreeByPath, add_fileinfo_to_symbols, demangle_symbol_names, extract_elf_symbols_fileinfo
from elfSizeAnalyze.misc.argumentParser import parse_args
from elfSizeAnalyze.misc.helpers import g
...
ERROR: No symbols from given section found or all were ignored!
    ...
    if args.rom:
        if args.json:
            create_json('ROM', filter_symbols(
    ...
    if args.ram:
        if args.json:
            create_json('RAM', filter_symbols(

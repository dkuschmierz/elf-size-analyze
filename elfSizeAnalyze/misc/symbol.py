...
from elfSizeAnalyze.misc.color import Color
from elfSizeAnalyze.misc.helpers import g, sizeof_fmt
from elfSizeAnalyze.misc.treeNode import TreeNode
...
class Symbol:
    ...
    def from_readelf_line(cls, line,
        ...
        m['size'] = int(m['size']) if m['size'].isdecimal() else int(
            m['size'], 16)
...
class SymbolsTreeByPath:
    ...
    def _generate_node_dict(self, min_size):
        ...
        for node, depth in self.tree_root.pre_order():
            ...
            iterNode = node
            nodePath.insert(
                0, iterNode.data.name if iterNode.is_symbol() else iterNode.data)
            while iterNode.parent.data != None:
                nodePath.insert(0, "childrens")
                nodePath.insert(0, iterNode.parent.data.name if iterNode.parent.is_symbol(
